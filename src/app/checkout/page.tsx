...
export default function CheckoutPage() {
  ...
  // ✅ FIX: Refs to handle polling cleanup and avoid stale closures
  ...
  useEffect(() => {
    ...
    // ✅ FIX: Cleanup intervals/timeouts if user navigates away
    return () => {
  ...
  // Format phone to Kenyan format: 2547XXXXXXXX
  ...
  // ✅ FIX: Validate phone format helper
  ...
  const handleMpesaPayment = async () => {
    ...
    try {
      ...
      // ✅ FIX: Initialize Supabase client inside the function to avoid stale client issues
      ...
      timeoutRef.current = setTimeout(() => {
        ...
        // ✅ FIX: Check current status via ref or functional update to avoid stale closure
  ...
  // ✅ FIX: Removed 'loading' from check since it was never set to false
  if (!isMounted) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-slate-900 via-slate-950 to-black text-white">
        <FaSpinner className="w-8 h-8 animate-spin text-cyan-400" />
      </div>
    )
  }
      ...
      <div className="max-w-md mx-auto">
        ...
        <div className="glass rounded-2xl p-6 mb-6">
          ...
          <input
            type="tel"
            value={phone}
            // ✅ FIX: Don't format aggressively on every keystroke to prevent cursor jumping
